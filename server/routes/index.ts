...
import { getNetworkKpis, getRouteInputs, getRouteMetrics } from '../lib/gold';
import { deriveAlerts } from '../db/bootstrap';
...
function actorOf(req: Request): string {
  return (
    (req.header('x-forwarded-email') as string) ||
    (req.header('x-forwarded-user') as string) ||
    'analista.demo'
  );
}
...
const CreateActionBody = z.object({
  ...
  tipoAccion: z.enum(['SUBIR', 'BAJAR', 'MANTENER', 'INVENTARIO']),
...
const DecideActionBody = z.object({
  accion: z.enum(['aprobar', 'rechazar', 'aplicar']),
...
const SimulateBody = z.object({
  ...
  elasticidad: z.number().min(0.1).max(3).optional(),
});
...
export function registerRoutes(appkit: AppKit): void {
  appkit.server.extend((app) => {
    // Identidad + como se ejecutan las consultas (transparencia para el asistente).
    ...
    // Resumen ejecutivo: KPIs de red (gold) + contadores transaccionales (Lakebase).
    app.get('/api/summary', async (_req, res) => {
      try {
        const kpi = await getNetworkKpis(appkit);
        const { rows: al } = await appkit.lakebase.query<{ c: string }>(
          "SELECT COUNT(*)::text AS c FROM revops.alerts WHERE estado = 'abierta'",
        );
        const { rows: ac } = await appkit.lakebase.query<{ c: string }>(
          "SELECT COUNT(*)::text AS c FROM revops.pricing_actions WHERE estado = 'propuesta'",
        );
        res.json({
          kpi,
        ...
        res.status(500).json({ error: `No pude construir el resumen: ${(err as Error).message}` });
      }
    });

    // Recomendacion de pricing para una ruta.
    app.get('/api/pricing/recommendation/:rutaId', async (req, res) => {
      ...
      try {
        ...
        res.json({ motor: pricingEngine.nombre, inputs, recomendacion: rec });
      } catch (err) {
        res.status(500).json({ error: (err as Error).message });
      }
    });

    // Simulador what-if de elasticidad.
    app.post('/api/pricing/simulate', async (req, res) => {
      ...
      try {
        ...
        res.json({ ruta: parsed.data.rutaId, base: { tarifaPropia: inputs.tarifaPropia, ocupacion: inputs.ocupacion, revenue30Usd: inputs.revenue30Usd }, simulacion: sim });
      } catch (err) {
        res.status(500).json({ error: (err as Error).message });
      }
    });

    // Listar acciones de pricing.
    app.get('/api/pricing/actions', async (req, res) => {
      try {
        const estado = typeof req.query.estado === 'string' ? req.query.estado : null;
        const sql = estado
          ? `SELECT * FROM revops.pricing_actions WHERE estado = $1 ORDER BY created_at DESC`
          : `SELECT * FROM revops.pricing_actions ORDER BY created_at DESC`;
    ...
    // Crear una propuesta de pricing.
      ...
      try {
        const { rows } = await appkit.lakebase.query(
          `INSERT INTO revops.pricing_actions
           ...
           RETURNING *`,
          [b.rutaId, b.cabina, b.claseTarifa, b.tipoAccion, b.tarifaAnteriorUsd, b.tarifaNuevaUsd, b.deltaPct, b.motivo, actor],
        );
        const created = rows[0];
        await audit(appkit, actor, 'crear_propuesta', 'pricing_action', String(created.id), { ruta: b.rutaId, tipo: b.tipoAccion, delta: b.deltaPct });
    ...
    // Aprobar / rechazar / aplicar una accion (loop analizar -> decidir -> actuar).
    app.patch('/api/pricing/actions/:id', async (req, res) => {
      ...
      const map: Record<string, { estado: string; ts: string }> = {
        aprobar: { estado: 'aprobada', ts: 'decided_at' },
        rechazar: { estado: 'rechazada', ts: 'decided_at' },
        aplicar: { estado: 'aplicada', ts: 'applied_at' },
      };
      const target = map[accion];
      try {
        const { rows } = await appkit.lakebase.query(
          `UPDATE revops.pricing_actions
             SET estado = $1, ${target.ts} = now(), aprobador = COALESCE($2, aprobador)
           WHERE id = $3 RETURNING *`,
          [target.estado, aprobador ?? actor, id.data],
        );
        if (rows.length === 0) {
    ...
    app.get('/api/alerts', async (req, res) => {
      try {
        const estado = typeof req.query.estado === 'string' ? req.query.estado : null;
        const sql = estado
          ? `SELECT * FROM revops.alerts WHERE estado = $1 ORDER BY created_at DESC`
          : `SELECT * FROM revops.alerts ORDER BY created_at DESC`;
        const { rows } = await appkit.lakebase.query(sql, estado ? [estado] : []);
        res.json(rows);
      } catch (err) {
        res.status(500).json({ error: (err as Error).message });
    ...
    // Recomputar alertas desde gold e insertar las nuevas (sin duplicar abiertas).
    app.post('/api/alerts/refresh', async (req, res) => {
      const actor = actorOf(req);
      try {
        const metrics = await getRouteMetrics(appkit);
        const candidates = deriveAlerts(metrics);
        let insertadas = 0;
        for (const c of candidates) {
          const { rows } = await appkit.lakebase.query<{ c: string }>(
            `SELECT COUNT(*)::text AS c FROM revops.alerts WHERE ruta_id=$1 AND tipo_alerta=$2 AND estado='abierta'`,
            [c.rutaId, c.tipo],
          );
        ...
        await audit(appkit, actor, 'refrescar_alertas', 'alert', 'batch', { evaluadas: candidates.length, insertadas });
        res.json({ evaluadas: candidates.length, insertadas });
      } catch (err) {
        res.status(500).json({ error: (err as Error).message });
      }
    });

    // Reconocer / cerrar una alerta.
    app.patch('/api/alerts/:id', async (req, res) => {
      ...
      try {
        const { rows } = await appkit.lakebase.query(
          `UPDATE revops.alerts SET estado = $1${setAck} WHERE id = $2 RETURNING *`,
          [nuevoEstado, id.data],
        );
        if (rows.length === 0) {
    ...
    app.get('/api/audit', async (req, res) => {
      try {
        ...
        const { rows } = await appkit.lakebase.query(
          `SELECT * FROM revops.audit_events ORDER BY created_at DESC LIMIT $1`,
          [limit],
        );
        res.json(rows);
      } catch (err) {
    ...
    app.post('/api/assistant', async (req, res) => {
      ...
      try {
        ...
        res.json({ proveedor: assistant.nombre, modo: assistant.modo, ...answer });
      } catch (err) {
        res.status(500).json({ error: (err as Error).message });
      }
    });
