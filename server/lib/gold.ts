...
import { AppKit, num, rows, str } from './appkit';

// Lecturas de las tablas gold (Delta) via el SQL Warehouse. El Service Principal
// consulta con sus propias credenciales. ruta_id siempre viaja como parametro
// tipado (sql.string), nunca concatenado.
...
export async function getRouteInputs(appkit: AppKit, rutaId: string): Promise<RouteInputs | null> {
  const res = await appkit.analytics.query(ROUTE_INPUTS_SQL, { ruta_id: sql.string(rutaId) });
  const r = rows(res)[0];
  if (!r) return null;
  return {
...
export interface NetworkKpis {
  ...
  raskProxyUsd: number; // revenue por ASK (asiento-km ofrecido)
  yieldPaxUsd: number; // revenue de pasaje por pasajero
  ancillaryPorPaxUsd: number;
  freshness: string; // fecha del ultimo vuelo volado
  periodoDias: number;
}
...
export async function getNetworkKpis(appkit: AppKit): Promise<NetworkKpis | null> {
  const res = await appkit.analytics.query(NETWORK_KPIS_SQL);
  const r = rows(res)[0];
  if (!r) return null;
  const rev = num(r.rev);
  ...
  return {
    ...
    revenueDeltaPct: prevRev > 0 ? Math.round(((rev - prevRev) / prevRev) * 1000) / 10 : 0,
    loadFactorProm: Math.round(lf * 10000) / 10000,
    loadFactorDeltaPts: Number.isFinite(prevLf) ? Math.round((lf - prevLf) * 1000) / 10 : 0,
    raskProxyUsd: ask > 0 ? Math.round((rev / ask) * 10000) / 10000 : 0,
...
export async function getRouteMetrics(appkit: AppKit): Promise<RouteMetric[]> {
  const res = await appkit.analytics.query(ROUTE_METRICS_SQL);
...
// Consultas puntuales para el asistente en modo demo.
export async function getRevenueMesUsd(appkit: AppKit): Promise<number | null> {
  const res = await appkit.analytics.query(
    `SELECT SUM(revenue_total_usd) AS rev FROM ${CATALOG}.flights WHERE estado='Volado' AND dias_a_salida >= -30`,
  );
  const r = rows(res)[0];
